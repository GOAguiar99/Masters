...
**Do not rewrite the remaining legacy chapters unless explicitly asked.** Title/metadata, Resumo/Abstract, Introduction (`chapters/0-Intro.tex`), and Background (`chapters/1-chapter.tex`) are already pivoted to the crash-aware topic; `chapters/2-chapter.tex` through `5-chapter.tex` are still the legacy DQ-WiFi draft.
