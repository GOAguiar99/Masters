...
**Do not rewrite the remaining legacy chapters unless explicitly asked.** Title/metadata, Resumo/Abstract, Introduction, Background, and Related Work are already pivoted to the crash-aware topic; `chapters/3-chapter.tex` through `5-chapter.tex` are still the legacy DQ-WiFi draft.
