...
The study evaluated later in this dissertation compares five Medium Access Control (MAC) policies on a shared IEEE~802.11p channel: a non-differentiated Distributed Coordination Function (DCF) baseline, Enhanced Distributed Channel Access (EDCA) with an explicit Differentiated Services Code Point (DSCP) to Voice mapping, and three adaptive extensions that temporarily suppress competing Best Effort (BE) traffic while crash alerts are active.
Those policies sit on top of a long standards path that runs from application criticality, through IP-layer marking, through 802.11 user priorities and EDCA queues, down to a contention-based radio that offers neither reserved slots nor link-layer acknowledgements for group-addressed frames.
...
Section~\ref{sec:bg:11p} specializes these mechanisms to IEEE~802.11p Outside the Context of a BSS (OCB) operation and contrasts Decentralized Congestion Control (DCC) with event-triggered suppression.
Section~\ref{sec:bg:dscp} traces the full DSCP-to-air-interface path that the study's classifier implements.
Section~\ref{sec:bg:sim} reviews the OMNeT++/Veins/INET and SUMO toolchain.
...
Cooperative Intelligent Transportation Systems (ITS) rely on wireless dissemination of safety-relevant information among nearby vehicles, roadside equipment, and, in some architectures, network backends.
Vehicle-to-Everything (V2X) denotes this family of exchanges and subsumes vehicle-to-vehicle (V2V), vehicle-to-infrastructure (V2I), vehicle-to-pedestrian (V2P), and vehicle-to-network (V2N) patterns.
Among the messages that circulate in such systems, two operational classes dominate the design of the lower layers.
\emph{Periodic awareness} traffic---for example Cooperative Awareness Messages (CAMs) in European ITS-G5 or Basic Safety Messages (BSMs) in Dedicated Short-Range Communications (DSRC)---is emitted continuously by every equipped station so that neighbors can maintain a local dynamic map.
\emph{Event-driven safety} traffic---crash notifications, hard-braking warnings, and related Decentralized Environmental Notification Messages (DENMs)---is rare, short-lived, and high-impact: its usefulness collapses if delivery is postponed until after the surrounding vehicles have already reacted, or failed to react, on the basis of their own sensors.
...
Service requirements for V2X scenarios are stated quantitatively in 3GPP TS~22.186~\cite{ts22186}.
...
The broader 5G service-requirements specification TS~22.261~\cite{3gpp_ts_22_261_r18} defines Ultra-Reliable Low-Latency Communication (URLLC) classes in general terms but defers the concrete V2X Key Performance Indicators (KPIs) to TS~22.186; citing the latter for platooning and related scenarios is therefore the accurate practice.
...
They do, however, establish the order of magnitude against which MAC-level tail shaping is later interpreted: when absolute Voice (VO) delays remain far below \SI{25}{\milli\second}, the evaluation claims headroom under synthetic saturation rather than a proven crash-mitigation benefit.
...
IEEE~802.11p adapts IEEE~802.11 for vehicular operation in the \SI{5.9}{\giga\hertz} band with \SI{10}{\mega\hertz} channelization and introduces OCB mode, in which stations exchange frames without association, authentication, or a Basic Service Set (BSS) context~\cite{jiang2008ieee80211p,IEEE80211}.
...
Chapter~\ref{sec:rel_work} surveys stronger alternatives---Time-Sensitive Networking (TSN)-inspired slotting, cooperative Time Division Multiple Access (TDMA), preemptive MAC designs, and cellular sidelink allocation---that relax this constraint; the background that follows explains why, within the constraint, EDCA alone is necessary but not sufficient.
...
The Distributed Coordination Function is the fundamental contention-based access method of IEEE~802.11~\cite{IEEE80211}.
...
DCF implements Carrier Sense Multiple Access with Collision Avoidance (CSMA/CA) with a binary slotted exponential backoff~\cite{bianchi2000DCF,IEEE80211}.
A station that has a frame to transmit first senses the medium.
If the channel has been idle for a Distributed Interframe Space (DIFS), the station may transmit immediately.
If the channel is busy, or becomes busy during the DIFS, the station defers: it draws a random backoff counter uniformly from the integer interval $[0, W-1]$, where $W$ is the current Contention Window (CW), and begins counting down only while the medium is sensed idle.
...
For unicast frames, a positive acknowledgement (ACK) confirms success; the absence of an ACK within an ACK Timeout triggers a retransmission after a new backoff drawn from a doubled contention window, up to a configured maximum.
The contention window starts at $\mathrm{CW}_{\min}$ and doubles after each unsuccessful transmission until it reaches $\mathrm{CW}_{\max}$; after a successful transmission, or after the retry limit is exhausted, it is reset.
Figure~\ref{fig:bg:dcf-backoff} illustrates a two-station contention episode, including a freeze interval caused by an overlapping transmission.
...
\begin{figure}[htbp]
  ...
  \vspace{1.8cm}}}
  \caption{Timeline of the IEEE~802.11 DCF backoff process, including carrier sensing, DIFS deferral, slotted countdown, and backoff freezing while the medium is busy.}
  \label{fig:bg:dcf-backoff}
\end{figure}
...
Second, every station that runs a single DCF queue with the same $\mathrm{CW}_{\min}$ and $\mathrm{CW}_{\max}$ has the same medium-access priority, regardless of the criticality of its traffic~\cite{mangold2003QoS,RFC8325_WiFi_QoS}.
DCF therefore provides no Quality-of-Service (QoS) differentiation whatsoever.

\subsection{Saturation throughput and the cost of contention}
\label{sec:bg:saturation}

Bianchi's classical analysis models the per-station backoff process under saturation---every station always has a packet ready to send---as a two-dimensional discrete-time Markov chain whose state is the pair $(\text{backoff stage}, \text{backoff counter})$~\cite{bianchi2000DCF}.
Under the approximation that each transmission attempt collides with a constant conditional probability $p$ independent of retransmission history, the chain yields a nonlinear system in $p$ and in the per-slot transmission probability $\tau$, with a unique solution.
From $\tau$ and $p$ one obtains the saturation throughput of the Basic Access and Request-to-Send/Clear-to-Send (RTS/CTS) modes.
...
Second, under Basic Access---the mode relevant to short safety frames and to group-addressed traffic that cannot use RTS/CTS---throughput degrades as the number of contending stations grows, because collisions waste large channel intervals on full-length frames~\cite{bianchi2000DCF}.
Third, the contention-window parameters that maximize throughput depend on the network size, yet $\mathrm{CW}_{\min}$ and the maximum backoff stage $m$ are fixed by the standard; in some regimes the resulting throughput is therefore ``significantly lower than the maximum achievable,'' and adaptive tuning of $W$ and $m$ is a natural response~\cite{bianchi2000DCF}.
...
IEEE~802.11e introduced the Hybrid Coordination Function (HCF) as the QoS-aware MAC coordination function, combining contention-based Enhanced Distributed Channel Access with the polling-based HCF Controlled Channel Access (HCCA)~\cite{IEEE80211e_2005,mangold2003QoS,IEEE80211}.
HCCA can bound delay under a single Hybrid Coordinator by granting Transmission Opportunities (TXOPs) according to a schedule, but it requires a coordinator, does not solve inter-cell interference without additional coordination, and is not available in fully decentralized OCB operation~\cite{mangold2003QoS,kosekszott2012What}.
...
EDCA maintains four access categories---Voice (\texttt{AC\_VO}), Video (\texttt{AC\_VI}), Best Effort (\texttt{AC\_BE}), and Background (\texttt{AC\_BK})---as parallel backoff entities within a station~\cite{IEEE80211e_2005,mangold2003QoS,kong2004EDCA}.
Each category has its own transmit queue and its own set of contention parameters: an Arbitration Interframe Space Number (AIFSN), a minimum and maximum contention window ($\mathrm{CW}_{\min}[\mathrm{AC}]$, $\mathrm{CW}_{\max}[\mathrm{AC}]$), and an optional TXOP limit.
The Arbitration Interframe Space of category $i$ is
\begin{equation}
  \mathrm{AIFS}[i] = \mathrm{SIFS} + \mathrm{AIFSN}[i]\cdot a\mathrm{SlotTime},
\end{equation}
...
Figure~\ref{fig:bg:edca-queues} summarizes the four-queue architecture and the mapping from IEEE~802.1D user priorities into access categories.

\begin{figure}[htbp]
  \centering
  \fbox{\parbox{0.92\linewidth}{\centering\vspace{1.8cm}
  \textbf{[Figure placeholder: EDCA four-queue architecture]}\\[0.4em]
  \textit{Show MSDUs entering four AC queues (VO, VI, BE, BK), each with its own AIFSN/CW/TXOP backoff entity, a virtual-collision resolver favoring higher ACs, and a single transmission onto the shared medium. Annotate the UP$\rightarrow$AC mapping (UP~6/7$\rightarrow$VO, UP~4/5$\rightarrow$VI, UP~0/3$\rightarrow$BE, UP~1/2$\rightarrow$BK).}
  \vspace{1.8cm}}}
  \caption{EDCA architecture: four parallel access-category queues with independent backoff entities, virtual-collision resolution, and the canonical user-priority to access-category mapping.}
  \label{fig:bg:edca-queues}
\end{figure}

Table~\ref{tab:bg:edca-params} reports representative EDCA parameter relationships as summarized by RFC~8325 for infrastructure WLANs; the numerical defaults used in the present study's INET configuration are stated in the System Model chapter and need not coincide with every published draft-era table~\cite{RFC8325_WiFi_QoS}.
...
\begin{table}[htbp]
  \centering
  \caption{Representative EDCA differentiation relative to Best Effort, following the UP$\rightarrow$AC and parameter discussion in RFC~8325. Absolute defaults vary by band and standard revision; the study's configured values are given in the System Model.}
  \label{tab:bg:edca-params}
  \begin{tabular}{@{}llccc@{}}
    \toprule
...
Romdhani, Ni, and Turletti proposed Adaptive EDCF, which adjusts persistence factors based on estimated collision rates to improve differentiation under load~\cite{romdhani2003AdaptiveEDCF}.
...
802.11aa introduced the Group Addressed Transmission Service (GATS), including GroupCast with Retries (GCR) Unsolicited Retry and GCR Block Ack, to raise the delivery probability of group-addressed frames that the base standard transmits without retries.
...
The mechanisms of Sections~\ref{sec:bg:dcf}--\ref{sec:bg:edca} are realized, in this work, on an IEEE~802.11p-like radio operating in OCB mode.
This section specializes the background to that environment and introduces Decentralized Congestion Control as the standardized, always-on counterpart to the study's event-triggered suppression.
...
In OCB mode, frames carry a wildcard BSSID; stations neither associate nor authenticate with an access point; and management procedures that assume a BSS are omitted~\cite{IEEE80211}.
The physical layer typically uses \SI{10}{\mega\hertz} OFDM channels in the \SI{5.9}{\giga\hertz} band, doubling the symbol duration relative to \SI{20}{\mega\hertz} 802.11a and improving robustness to multipath and Doppler at highway speeds~\cite{jiang2008ieee80211p,etsi_en_302663}.
Figure~\ref{fig:bg:ocb} contrasts BSS-mode infrastructure Wi-Fi with OCB vehicular operation.

\begin{figure}[htbp]
  \centering
  \fbox{\parbox{0.92\linewidth}{\centering\vspace{1.8cm}
  \textbf{[Figure placeholder: BSS infrastructure Wi-Fi vs.\ 802.11p OCB]}\\[0.4em]
  \textit{Left: stations associated to an AP inside a BSS, with uplink/downlink and AP-coordinated services. Right: vehicles in OCB mode exchanging direct V2V frames on a shared 5.9~GHz / 10~MHz channel with no association, no AP, and no central scheduler. Annotate ``wildcard BSSID'' and ``distributed contention only.''}
  \vspace{1.8cm}}}
  \caption{Comparison of infrastructure BSS operation and IEEE~802.11p Outside the Context of a BSS (OCB) vehicular operation. Prioritization in OCB must be realized locally; AP-centric QoS mechanisms do not apply.}
  \label{fig:bg:ocb}
\end{figure}
...
European ITS-G5 deployments complement EDCA with Decentralized Congestion Control at the access layer~\cite{etsi_dcc_2018}.
DCC measures the Channel Busy Ratio (CBR)---the fraction of time the medium is sensed busy---and applies transmit power control (TPC), transmit rate control (TRC), and DCC access control so that each station's contribution to channel load remains within a target operating region.
...
This section traces that mapping from the Differentiated Services architecture, through the Expedited Forwarding per-hop behavior and DSCP~46, through the DSCP-to-user-priority-to-access-category path standardized for Wi-Fi, to the multicast limitations that still apply after a correct mapping.
...
\begin{figure}[htbp]
  ...
  \vspace{1.8cm}}}
  \caption{Cross-layer path from IP DSCP marking to EDCA channel access. Correct Voice treatment of Expedited Forwarding traffic requires an explicit DSCP-to-user-priority mapping; the default three-MSB heuristic places EF in Video.}
  \label{fig:bg:dscp-path}
\end{figure}

\subsection{Differentiated Services architecture}
\label{sec:bg:diffserv}

RFC~2474 defines the Differentiated Services (DS) field in IPv4 and IPv6 headers: six bits form the DSCP used to select a Per-Hop Behavior (PHB) at each node, and DS-compliant nodes must select PHBs by matching the entire six-bit field against a configurable mapping~\cite{RFC2474_DSField}.
...
RFC~3246 defines the Expedited Forwarding PHB as a building block for low-delay, low-jitter, and low-loss services, obtained by ensuring that the EF aggregate is served at a configured rate independently of the non-EF load~\cite{RFC3246_EF_PHB}.
...
Marking them with DSCP~46 and admitting them to the most aggressive MAC category is therefore the DiffServ-consistent modeling choice for the two-class abstraction, not a claim that production ITS stacks universally use EF for DENM-like messages.
...
Table~\ref{tab:bg:dscp-map} summarizes the default pitfall and the recommended Voice mapping for the two codepoints used in this study.

\begin{table}[htbp]
  \centering
  \caption{DSCP to user-priority to access-category mapping for the study's two traffic classes, contrasting the common three-MSB default with the RFC~8325 recommendation for Expedited Forwarding.}
  \label{tab:bg:dscp-map}
  \begin{tabular}{@{}lllll@{}}
    \toprule
...
First, because there are no ACKs for multicast packets, the transmitter cannot know whether a retransmission is needed; there is consequently no loss-driven backoff on behalf of failed receivers, and packet error rates of 5\% or more are not uncommon~\cite{RFC9119_Multicast_IEEE802_Wireless}.
...
Table~\ref{tab:bg:multicast-tools} summarizes which reliability tools are available in infrastructure BSS operation versus in the OCB setting of this study.
...
\begin{table}[htbp]
  \centering
  \caption{Link-layer reliability mechanisms for group-addressed frames: availability in infrastructure BSS Wi-Fi versus in IEEE~802.11p OCB operation as used in this study.}
  \label{tab:bg:multicast-tools}
...
Discrete-event simulation is therefore the dominant methodology for Inter-Vehicle Communication (IVC) protocol evaluation, provided that both the network stack and the mobility process are represented at adequate fidelity~\cite{veins_sommer2011}.
...
In this dissertation, INET provides the IEEE~802.11p-capable MAC and PHY baseline, IP forwarding, and the hooks into which the study's traffic generators, DSCP classifier, and adaptive MAC controller are inserted.
...
Sommer, German, and Dressler argue that faithful IVC evaluation therefore requires \emph{bidirectional} coupling of a network simulator and a road-traffic microsimulator, and introduce Veins (Vehicles in Network Simulation) as a hybrid framework that joins OMNeT++ to the Simulation of Urban MObility (SUMO) package through the Traffic Control Interface (TraCI)~\cite{veins_sommer2011}.
Veins allows the network model to control vehicle entities (for example, stopping a designated crash node) while SUMO continuously updates positions, headings, and neighborhood relations that feed the radio model.
...
Figure~\ref{fig:bg:veins} sketches the closed loop used in this work: SUMO advances vehicle mobility; TraCI exposes state and control; Veins/INET execute the 802.11p stack and the study's QoS modules; and KPIs are recorded from packet timestamps and MAC counters.
...
\begin{figure}[htbp]
  ...
  \vspace{1.8cm}}}
  \caption{Bidirectional coupling of SUMO microscopic mobility and OMNeT++/INET network simulation through Veins and TraCI, including the study-specific traffic and MAC modules.}
  \label{fig:bg:veins}
\end{figure}
...
Physical-layer assumptions---free-space path loss, binary obstacle loss, fixed SNIR threshold---are held constant across MAC policies so that measured differences isolate contention and queueing.
...
\begin{enumerate}
  ...
  Cooperative V2X warnings are useful only if delivered promptly to nearby vehicles; 3GPP TS~22.186 places a representative platooning bound at \SI{25}{\milli\second} end-to-end~\cite{ts22186}.
  On IEEE~802.11p in OCB mode, those warnings share a decentralized channel with no central scheduler~\cite{jiang2008ieee80211p,IEEE80211}.
...
Chapter~\ref{sec:rel_work} surveys stronger contention-control proposals---synchronized slotting, cooperative TDMA, preemptive and UAV-assisted MACs, and cellular sidelink allocation---and positions the present event-driven approach against them.
Chapter~\ref{chap:sysmodel} then formalizes the two-class traffic model, the crash timeline, and the five MAC policies that instantiate the background developed here.
