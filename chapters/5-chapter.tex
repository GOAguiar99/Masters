...
At heavy \texttt{netload\_high} every policy sits near 2.2~ms Voice P95---the high-load burst profile with a \SI{5}{\milli\second} inter-copy gap; low/medium profiles quantize alert age to about 4.6--4.9~ms instead (\autoref{sec:eval:light}).
...
\begin{figure}[htb]
	\caption{\label{fig:eval:regime_contrast}Voice P95 delay by policy at high load under uniform Poisson offer versus hotspot contention (100~vehicles, three-seed means). Protection is inert under diffuse load and selective under concentrated near-crash airtime: only emergency compresses the hotspot tail.}
	\begin{center}
		\includegraphics[width=0.95\linewidth]{Figs/fig_10_regime_vo_p95_contrast_highway_heavy.pdf}
	\end{center}
	\fonte{Prepared by the author.}
\end{figure}
...
The mechanism is visible in the controller counters: under the 20~ms $\times$~8-copy alert stream the blocked duty cycle is near-continuous (about $16.1\times10^{3}$ Best Effort frames dropped while blocked plus about one thousand suppressed grants), so the hot queues never refill the arena during the window.
...
\begin{table}[H]
	\ABNTEXfontereduzida
	\caption{\label{tab:eval:hotspot}Hotspot high-load Voice protection results (100~vehicles, hotspot nodes 1--3 streaming 1200~B, three-run means). \emph{VO \gls{mac} drops} are Voice-attributed incorrectly-received counts summed over nodes; \emph{\gls{be} from crash} is the same-sender Best Effort mean delay (starvation under EDCA/\texttt{stable}/\texttt{guarded}); \emph{\gls{be} dropped blocked} counts emergency preemptions.}
	\begin{center}
		\scriptsize
		...
		\begin{tabular}{@{}lrrrrrr@{}}
			...
			EDCA      & 4.448 & \shortstack{19.96\\[-1pt]{\scriptsize [18.7--20.7]}} & 9.624 & 25.2 & 1453 & 0 \\
			Stable    & 4.608 & \shortstack{21.04\\[-1pt]{\scriptsize [18.8--22.8]}} & 9.636 & 25.4 & 1386 & 0 \\
			Guarded   & 4.439 & \shortstack{20.04\\[-1pt]{\scriptsize [19.8--20.4]}} & 9.689 & 25.1 & 981 & 0 \\
...
\texttt{stable} and \texttt{guarded} keep Voice P95 on the EDCA baseline (20--21~ms): their bounded blocking never clears the hot queues.
...
\begin{figure}[htb]
	...
	\begin{center}
		\includegraphics[width=0.92\linewidth]{Figs/fig_01_p95_delay_priority_gap_highway_heavy.pdf}
	\end{center}
	\fonte{Prepared by the author.}
\end{figure}

Alert age under this burst schedule is quantized to multiples of the inter-copy gap~$g$: if the first successful physical copy is copy~$k$, measured age is $k\cdot g + \delta$ with $\delta \approx \SIrange{0.2}{0.7}{\milli\second}$~\cite{Robust_Broadcast_Ma,Reliable_Broadcast_Farnoud}.
Contention shifts probability mass between quanta; it does not create a \gls{mac}-policy lever under diffuse load, which is why Voice P95 tracks $g$ (about 4.6--4.9~ms at low/medium gaps, about 2.2~ms at the 5~ms high-load gap) rather than the access category.
...
\begin{figure}[htb]
	\caption{\label{fig:eval:gain_cost}Voice-gain versus Best Effort cost relative to plain \gls{dcf} at high load (three-seed means). Dual panels use independent axes: left is uniform \texttt{netload\_high}; right is \texttt{hotspot\_high}. Negative $\Delta$ Voice P95 is an improvement; positive $\Delta$ Best Effort P95 is a penalty.}
	\begin{center}
		\includegraphics[width=0.95\linewidth]{Figs/fig_04_vo_gain_vs_be_cost_highway_heavy.pdf}
	\end{center}
	\fonte{Prepared by the author.}
\end{figure}
...
\begin{figure}[htb]
	...
	\begin{center}
		\includegraphics[width=0.88\linewidth]{Figs/fig_07_v2x_control_actions_by_load_highway_heavy.pdf}
	\end{center}
	\fonte{Prepared by the author.}
\end{figure}
...
The uniform Poisson matrix is the null that makes that result conditional: Voice alert age stays on the burst-schedule floor for every policy, while adaptive blocking mainly taxes Best Effort (\autoref{sec:eval:light}).
