...
\autoref{chap:background} established that \gls{edca} improves statistical priority for Voice traffic but cannot guarantee delay or throughput bounds on a shared wireless medium~\cite{kosekszott2012What,mangold2003QoS}, that correct \gls{dscp} to access-category mapping is not automatic~\cite{RFC8325_WiFi_QoS}, and that group-addressed IEEE~802 frames inherit a hostile link-layer contract without acknowledgement or retransmission~\cite{RFC9119_Multicast_IEEE802_Wireless}.
...
Some proposals pursue \emph{stronger contention control} than plain EDCA through slot scheduling, preemption, cooperative \gls{tdma}, or cellular sidelink allocation.
Others study \emph{when} traffic is allowed to contend and what ordinary traffic pays when urgent flows are favored.
A third family regulates offered load \emph{continuously} through adaptive EDCA parameters or \gls{dcc}.
A fourth transplants \gls{tsn} and \gls{ofdma} ideas from wired or infrastructure Wi-Fi into wireless settings.
\autoref{fig:rw:taxonomy} situates these families in a design space whose axes are synchronization or infrastructure dependency and always-on versus event-triggered intervention.
The figure is a schematic positioning aid: markers are placed qualitatively from each work's stated assumptions, not from a numerical score.

\begin{figure}[htb]
	\caption{\label{fig:rw:taxonomy}Schematic design-space taxonomy of related Medium Access Control approaches. Axes contrast local / infrastructure-free operation with synchronization, infrastructure, or cellular machinery, and event-triggered / short-lived intervention with always-on / permanent regulation. Marker positions are qualitative judgments from each work's stated assumptions, not computed scores; this dissertation occupies the local, event-triggered region.}
	\begin{center}
		\includegraphics[width=0.95\linewidth]{Figs/rw_taxonomy.pdf}
	\end{center}
	\fonte{Prepared by the author.}
\end{figure}

This dissertation occupies a deliberately narrow point in that space: explicit DSCP~46-to-Voice mapping plus local, event-triggered \gls{be} suppression on IEEE~802.11p \gls{ocb} operation, without reservations, clock synchronization, or protocol changes~\cite{jiang2008ieee80211p,IEEE80211}.
The remainder of the chapter surveys the four families above, synthesizes their assumptions and limitations, and states the research gap that \autoref{chap:sysmodel} instantiates as a five-policy evaluation ladder.
...
In \gls{omnet}/\gls{inet} experiments, short slot lengths (for example, \SI{2}{\milli\second}) are reported to reduce average collision rates relative to a CSMA/CA baseline under the evaluated platooning scenarios~\cite{TSNCtl_Feraudo}.
...
Li et al.\ address \gls{bsm} delivery under high vehicle density by combining IEEE~802.11 CSMA/CA with a preemptive-resume priority scheme and an arbitration device, in a setting where a \gls{uav} acts as a receiver for BSM uploads toward \gls{rsu} processing~\cite{PRP_MAC_Li}.
...
Linn, Liu, and Gao propose CFC-MAC, a distributed contention-free cooperative MAC for \glspl{vanet} that combines cooperative forwarding, vectorized trajectory estimation, and time-slot reallocation to resolve heterogeneous collisions under stochastic traffic~\cite{CFC_MAC_Linn}.
...
Thabet, Bendary, and Elbayoumy study adaptive autonomous resource allocation for 5G New Radio V2X (NR-V2X) sidelink, improving legacy \gls{sps} behavior for periodic safety-message transmissions in decentralized Mode~2 operation~\cite{CV2X_Sidelink_Allocation}.
...
The present dissertation drops \gls{tas}-oriented scheduling, abandons synchronization as a requirement, and focuses on a shared OCB channel in which crash-alert Voice bursts compete with ordinary BE multicast under five MAC policies that require no gate-control lists~\cite{11288825}.

\subsection{Synthesis: stronger control under broader assumptions}
\label{sec:rw:stronger-synth}

\autoref{tab:rw:comparison} summarizes the approaches in this section and the families reviewed later.
...
\begin{table}[htb]
	\ABNTEXfontereduzida
	\caption{\label{tab:rw:comparison}Comparative summary of related approaches and this dissertation. Assumptions and mechanisms are those stated by the cited works; the final row positions the crash-aware study.}
	\begin{center}
		\small
  
  
		  \setlength{\tabcolsep}{2.5pt}
		  \begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{2.35cm}>{\RaggedRight\arraybackslash}p{2.35cm}>{\RaggedRight\arraybackslash}p{2.2cm}>{\RaggedRight\arraybackslash}p{1.9cm}>{\RaggedRight\arraybackslash}p{2.35cm}>{\RaggedRight\arraybackslash}p{2.2cm}@{}}
		    \toprule
		    Approach & Key mechanism & Require\-ments & Target traffic & Advantages (claimed) & Relation to this work \\
		    \midrule
		    TSNCtl\newline\cite{TSNCtl_Feraudo} & App-level slot schedule + FSM & Sync, platoon master & Platoon V2V & Lower collisions vs CSMA/CA & Sync/leader; preprint \\
		    PRP-MAC\newline\cite{PRP_MAC_Li} & Preemptive-resume priority + arbiter & UAV/RSU-assisted stack & BSM uploads & Lower BSM delay & Infra + richer MAC \\
		    CFC-MAC\newline\cite{CFC_MAC_Linn} & Cooperative TDMA + slot repair & Slot sync, coop.\ signaling & VANET safety & Low collision, determ.\ access delay & TDMA contract \\
		    NR-V2X Mode~2\newline\cite{CV2X_Sidelink_Allocation} & Adaptive SPS / resource selection & Cellular sidelink & Periodic safety & Better resource use vs legacy SPS & Different RAT \\
		    SkipCCH\newline\cite{SkipCCH_Garrido} & Avoid \gls{cch}-induced \gls{sch} bursts & IEEE~1609.4 multi-channel & Video + QoS ACs & Less contention, less low-AC starvation & Multi-channel video \\
		    Adaptive EDCF\newline\cite{romdhani2003AdaptiveEDCF} & CW/\-persistence retuning & Continuous measurement & Differen\-tiated ACs & Better differentiation under load & Always-on, aggregate \\
		    DCC\newline\cite{etsi_dcc_2018} & CBR-driven TPC/\-TRC/\-access & Always-on ITS-G5 loop & Aggregate load & Congestion management & Comple\-mentary \\
		    Wireless TSN / OFDMA\newline\cite{seliem2023wirelesstsn,schneider_trigger_2024} & TAS/TWT/RU scheduling & Wi-Fi~6/NIC mods or sync & Industrial / scheduled & Tighter latency control & Out of OCB scope \\
		  ...
		  \end{tabular}
	\end{center}
	\fonte{Prepared by the author from the cited works.}
\end{table}
...
As noted in \autoref{chap:background}, their absolute delay numbers correspond to a lightly loaded configuration and are not transferable to the saturated BE loads of this study; the transferable claim is the trend.
...
That philosophy also differs from the always-on congestion loops of adaptive EDCA and DCC discussed in Section~\ref{sec:rw:alwayson}: those mechanisms retune aggregate load continuously, whereas BE suppression is armed only for the crash-alert window and then released.
...
European ITS-G5 deployments standardize \gls{dcc} at the access layer~\cite{etsi_dcc_2018}.
As summarized in \autoref{chap:background}, DCC measures the \gls{cbr} and applies transmit power control, transmit rate control, and DCC access control so that each station's contribution to channel load remains inside a target region; reactive and adaptive algorithm variants are defined~\cite{etsi_dcc_2018}.
...
Those are the questions reserved for the five-policy matrix in \autoref{chap:sysmodel}.
