...
Thabet, Bendary, and Elbayoumy study adaptive autonomous resource allocation for V2X sidelink, with the paper framed toward 6G; the Mode~2 sensing and semi-persistent scheduling mechanisms they adapt are those of 5G New Radio V2X (NR-V2X), improving legacy \gls{sps} behavior for periodic safety-message transmissions in decentralized operation~\cite{CV2X_Sidelink_Allocation}.
...
Assumptions and mechanisms are those stated by the cited works; the final row positions the crash-aware study.
...
\begin{table}[htb]
	\ABNTEXfontereduzida
	\caption{\label{tab:rw:comparison}Comparative summary of related approaches and this dissertation.}
	\begin{center}
		...
		\begin{tabularx}{\linewidth}{@{}*{6}{>{\RaggedRight\arraybackslash}X}@{}}
		    ...
		    NR-V2X Mode~2\newline\cite{CV2X_Sidelink_Allocation} & Adaptive SPS / resource selection & Cellular sidelink & Periodic safety & Better resource use vs legacy \gls*{sps} & Different radio access technology \\
		    SkipCCH\newline\cite{SkipCCH_Garrido} & Avoid \glsentryshort{cch}-induced \glsentryshort{sch} bursts & IEEE~1609.4 multi-channel & Video + QoS ACs & Less contention, less low-AC starvation & Multi-channel video \\
		    Adaptive EDCF\newline\cite{romdhani2003AdaptiveEDCF} & CW/persistence retuning & Continuous measurement & Differentiated ACs & Better differentiation under load & Always-on, aggregate \\
		    DCC\newline\cite{etsi_dcc_2018} & CBR-driven TPC/TRC/access & Always-on ITS-G5 loop & Aggregate load & Congestion management & Complementary \\
