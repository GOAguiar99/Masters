...
Each logical alert may be repeated a configurable number of times, with configurable spacing and jitter, to represent short warning bursts; such application-level repetition of safety messages is the reliability mechanism on which robust broadcast schemes for vehicular networks are built~\cite{Robust_Broadcast_Ma,Reliable_Broadcast_Farnoud,Cooperative_Broadcast_Yoo}.
...
\begin{figure}[htb]
	\caption{\label{fig:sys:stack}Conceptual cross-layer path for crash-aware prioritization. Study-specific mechanisms act at Application, Network/\glsentryshort{qos}, and \glsentryshort{mac}; the shared \glsentryshort{phy} baseline is identical across all policies.}
	\begin{center}
...
Quantitative Voice or Best Effort delay statements belong to the evaluation, not to closed-form bounds from this chapter.

Together, these definitions constitute the crash-aware system studied below.
\autoref{chap:implementation} maps them onto the software artifact and its parameter bindings; \autoref{chap:evaluation} then measures Voice service under contention and the Best Effort cost of that protection.
