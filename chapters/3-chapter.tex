...
\begin{figure}[htb]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{2.4cm} X}
		...
		\addlinespace
		\emph{blocking} &
		  Entered when the local Voice queue crosses the activation threshold or a Voice-marked frame is overheard.
...
It moves to \emph{sending} while its own Voice frames obtain the channel and returns to \emph{listening} once the finite protection window and a short guard interval expire.
The transition strip in \autoref{fig:sys:fsm} shows the nominal cycle; two further transitions exist in the realization: a node in \emph{listening} moves directly to \emph{sending} when its own Voice frame wins the channel without prior threshold crossing, and a node in \emph{sending} falls back to \emph{blocking} when the send guard expires while the protection window is still open.
