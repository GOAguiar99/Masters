...
\begin{table}[htb]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}p{3.2cm} X}
		...
		\addlinespace
		\texttt{v2x\_vo\_stable} & Sustained \gls{vo} protection & Inherits \texttt{edca\_only}. Block~\SI{15}{\milli\second}; max continuous~\SI{80}{\milli\second}; send guard~\SI{5}{\milli\second}; \gls{vo} threshold~2. \\
		\addlinespace
		\texttt{v2x\_vo\_guarded} & Bounded suppression & Same inheritance. Block~\SI{4}{\milli\second}; max continuous~\SI{20}{\milli\second}; send guard~\SI{4}{\milli\second}; \gls{vo} threshold~3. \\
		\addlinespace
		\texttt{v2x\_vo\_emergency} & Emergency preemption & Same inheritance with emergency preemption. Block~\SI{10}{\milli\second}; max continuous~\SI{60}{\milli\second}; send guard~\SI{5}{\milli\second}; \gls{vo} threshold~1; may drop \gls{be} while protection is active. \\
		\bottomrule
	\end{tabularx}
	\end{center}
	\fonte{Prepared by the author.}
