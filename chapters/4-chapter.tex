...
\gls{inet}~4.5.4 supplies the Internet protocol stack, IEEE~802.11p-capable \gls{mac} and \gls{phy}, radio propagation abstractions, and the \gls{hcf} into which the study's adaptive controller is substituted for selected policies~\cite{inet_framework,IEEE80211e_2005}.
...
\begin{figure}[htb]
	\caption{\label{fig:impl:stack}Implemented cross-layer path from study applications to the shared radio. Adaptive \glsentryshort{hcf} modules appear only under the \texttt{edca\_v2x\_vo\_*} policies.}
	\begin{center}
		\includegraphics[width=0.95\linewidth]{Figs/impl_stack.pdf}
	\end{center}
	\fonte{Prepared by the author.}
\end{figure}
...
\begin{table}[htb]
	...
	\begin{center}
		\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{4.2cm} >{\raggedright\arraybackslash}X@{}}
			...
			\addlinespace
			\texttt{localPort}/\texttt{destPort}\newline(9001) &
...
Copy~$i$ of a sequence is scheduled at $\max(0,\; i\cdot\texttt{repeatGap} + U(-J,\,J))$ after the logical burst time, where $J$ is \texttt{repeatJitter}.
The jitter therefore applies to the \emph{first} copy as well as to later repeats, so even an alert delivered on its first transmission carries a dispatch offset whose 95th percentile is $0.9\,J$.
Because the same logical burst time is the origin of the alert-age metric (\autoref{sec:impl:signals}), this offset is a lower bound on measured Voice delay that no \gls{mac} policy can remove; \autoref{sec:eval:light} quantifies its effect on the reported Voice tails.
...
\begin{table}[htb]
	...
	\begin{center}
		\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{4.2cm} >{\raggedright\arraybackslash}X@{}}
			...
			\addlinespace
			\texttt{crashAt}\newline\texttt{resumeAfter} &
			  Absolute crash start and crash-window duration. \\
			\addlinespace
			\texttt{sendInterval}\newline\texttt{payloadBytes} &
			  Inter-burst period and payload during the window. \\
			\addlinespace
			\texttt{repeatCount}\newline\texttt{repeatGap}\newline\texttt{repeatJitter} &
			  Physical repeats of each logical alert. Jitter is drawn per copy, including the first, so every copy carries a dispatch offset relative to the logical burst time. \\
			\addlinespace
			\texttt{localPort}~9002\newline\texttt{destPort}~9001 &
			  TX on a dedicated local port; RX KPIs collected by Crit on~9001. \\
			\addlinespace
			Statistics &
			  \texttt{voTxPackets} (physical),\newline\texttt{voLogicalTxPackets} (per sequence). \\
			\bottomrule
		\end{tabularx}
	\end{center}
	\fonte{Prepared by the author.}
...
\begin{table}[htb]
	\ABNTEXfontereduzida
	\caption[Adaptive controller parameters exposed by \texttt{V2xHcf}]{\label{tab:impl:v2xhcf}Adaptive controller parameters exposed by \texttt{V2xHcf} / \texttt{V2xEdcaFsmController}.}
	\begin{center}
		\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{4.2cm} >{\raggedright\arraybackslash}X@{}}
			\toprule
			\textbf{Parameter} & \textbf{Meaning} \\
			\midrule
			\path{adaptiveBlocking} &
			  Enables Voice-driven Best Effort suppression through the FSM. \\
			\addlinespace
			\path{emergencyPreemption} &
			...
			\addlinespace
			\path{maxContinuousBlock} &
			  Cap on one continuous blocking/sending interval (also mirrored on the FSM). \\
			\addlinespace
			\path{voQueueThreshold} &
			  Local Voice queue depth that can trigger protection from upper traffic. \\
			\addlinespace
			\path{FSMController.}\newline
			\path{sendingGuardTimeout} &
			  Fallback timer armed on Voice TX start; if TX end does not fire first, leave \emph{sending} according to whether the protection window is still open. \\
			\bottomrule
		\end{tabularx}
	\end{center}
	\fonte{Prepared by the author.}
...
\begin{table}[htb]
	\ABNTEXfontereduzida
	\caption{\label{tab:impl:hooks}Principal integration hooks of \texttt{V2xHcf} relative to the stock INET \glsentryshort{hcf}.}
	\begin{center}
		...
		\begin{tabularx}{\linewidth}{@{}
			...
			@{}}
			\toprule
			\textbf{Hook} & \textbf{Trigger} & \textbf{Effect} \\
			\midrule
			Voice demand detection &
			  Local Voice enqueue above \path{voQueueThreshold}, or an overheard Voice-classified \gls*{mac} frame. &
			  Calls \path{onVoDemandDetected}; may start or extend a protection window of length \path{blockDuration}, capped by \path{maxContinuousBlock}. \\
			\addlinespace
			Upper-frame gating &
			  FSM reports Best Effort blocked (\emph{blocking} or \emph{sending}). &
			  Best Effort channel-access requests are deferred. \\
			\addlinespace
			Own Voice on air &
			  Start and end of the node's own Voice transmissions. &
			  On TX start, FSM enters \emph{sending}. On TX end (or send-guard timeout), it returns to \emph{blocking} if the window is still open, otherwise to \emph{listening}. \\
			\addlinespace
			Emergency preemption &
			  \path{emergencyPreemption} is true and protection is active. &
			  Newly arriving Best Effort frames may be dropped with a congestion reason; stale Best Effort grants suppressed. \\
			\addlinespace
			Instrumentation &
			  Controller and FSM events during a run. &
			  Signals/scalars \path{beDroppedWhileBlocked}, \path{beGrantSuppressedWhileBlocked}, \path{voProtectionActivation}; FSM vector \path{v2xState} (0~=~listening, 1~=~blocking, 2~=~sending). \\
			\bottomrule
		\end{tabularx}
	\end{center}
	\fonte{Prepared by the author.}
...
\begin{table}[htb]
	...
	\begin{center}
		\begin{tabularx}{\linewidth}{@{}
			...
			\midrule
			App. &
			  Multicast UDP hooks in \path{VeinsInetApplicationBase} &
			  \path{CritPacketSender}, \path{CrashBurstApp}; KPI tags and deduplication \\
			\addlinespace
			Net/\gls*{qos} &
			  IPv4 forwarding and transport &
			  \path{QosClassifier} (\gls*{dscp}~46~$\rightarrow$ Voice \gls*{up}) \\
			\addlinespace
			\gls*{mac} &
			  IEEE~802.11p \gls*{dcf}/\gls*{edca} queues and channel access &
			  \path{V2xIeee80211Mac}; \path{V2xHcf}+\path{V2xEdcaFsmController} on adaptive policies \\
			\addlinespace
			\gls*{phy} &
...
\begin{itemize}
  ...
  \item \glsentryshort{msdu} aggregation policy cleared (\texttt{msduAggregationPolicy.typename = ""});
...
Clearing the aggregation policy disables \gls{msdu} aggregation, so each application packet maps to exactly one transmitted frame and MAC-level counters stay comparable across policies.
...
\begin{table}[htb]
	...
	\begin{center}
		...
		\begin{tabularx}{\linewidth}{@{}l *{5}{>{\centering\arraybackslash}X}@{}}
			\toprule
			\textbf{Config} &
			\shortstack{\texttt{block}\\[-1pt]\texttt{Duration}} &
			\shortstack{\texttt{maxContinuous}\\[-1pt]\texttt{Block}} &
			\shortstack{\texttt{sendingGuard}\\[-1pt]\texttt{Timeout}} &
			\shortstack{\texttt{voQueue}\\[-1pt]\texttt{Threshold}} &
			\shortstack{\texttt{emergency}\\[-1pt]\texttt{Preemption}} \\
			\midrule
			\texttt{stable} &
			  \SI{15}{\milli\second} & \SI{80}{\milli\second} & \SI{5}{\milli\second} & 2 & false \\
			\texttt{guarded} &
			  \SI{4}{\milli\second} & \SI{20}{\milli\second} & \SI{4}{\milli\second} & 3 & false \\
			\texttt{emergency} &
			  \SI{10}{\milli\second} & \SI{60}{\milli\second} & \SI{5}{\milli\second} & 1 & true \\
			\bottomrule
		\end{tabularx}
	\end{center}
	\fonte{Prepared by the author.}
...
Best Effort inter-arrivals are Poisson: OMNeT++ \texttt{exponential($\cdot$)} is marked \texttt{volatile} and redrawn independently before every packet, so the mean is a rate rather than a per-vehicle constant period.
Voice burst periods are fixed, with optional per-repeat jitter.
...
\begin{xltabular}{\linewidth}{@{}
	...
	>{\raggedright\arraybackslash}X
	@{}}
	\caption{\label{tab:impl:parameters}Simulation parameters used in the crash-aware Veins/\glsentryshort{inet} artifact.}\\
	\toprule
	...
	\midrule
	\endfirsthead
	\caption[]{Simulation parameters used in the crash-aware Veins/\glsentryshort{inet} artifact. (continued)}\\
	\toprule
	\textbf{Parameter} & \textbf{Value} & \textbf{Notes} \\
	\midrule
	\endhead
	...
	Density &
	  10~/~100~veh. &
	  Light and heavy packages (\path{veins_inet_highway_light}/\texttt{\_heavy}); same \texttt{omnetpp.ini}, different \texttt{highway.rou.xml}. \\
	\addlinespace
	Crash &
	  \SI{30}{\second}--\SI{60}{\second} &
	  Node index~0 (\path{node0_highway}): TraCI stop, DSCP~46 alerts; BE background continues. \\
	\addlinespace
	Radio &
	  802.11p / ch.~3 &
	  5.9~GHz, 10~MHz, 20~mW; \path{Ieee80211DimensionalRadio}; mode~\texttt{p}; data frames at 12~Mbit/s (fastest mandatory mode, \gls*{inet} default rate selection). \\
	\addlinespace
	Propagation &
	  FS~+~binary &
	  Free-space path loss; no fading/shadowing; \path{IdealObstacleLoss} + \texttt{highway.obstacles.xml}; SNIR~\SI{4}{\decibel}; sens.\ $-$85~dBm; noise $-$110~dBm (\gls*{inet}~4.5.4 defaults, not overridden). \\
	\addlinespace
	Multicast &
	  \texttt{224.0.0.1} &
	...
	TraCI &
	  port~9999 &
	  \texttt{localhost}; \path{VeinsInetManager} + \path{veins_launchd}; update~\SI{1}{\second}; SUMO step~\SI{0.1}{\second}. \\
	\addlinespace
	Host/\gls*{mac} &
	  \path{VeinsInetCar} &
	  Always \path{V2xIeee80211Mac}; adaptive policies replace \texttt{Hcf} with \path{V2xHcf}. \\
	\addlinespace
	Classifier &
	  DSCP~46~$\rightarrow$~VO &
	  \path{QosClassifier} on EDCA policies only (\texttt{crashDscp=46}, \texttt{defaultUp=0}); unused under \texttt{plain}. \\
	\addlinespace
	Recording &
	  vec.\ +\ sca. &
	...
	\texttt{edca\_only} &
	  Stock EDCA &
	  \texttt{Hcf} + \path{QosClassifier}. DSCP~46~$\rightarrow$~\texttt{AC\_VO}, else UP~0. Queues BK/BE/VI/VO: 128/128/64/32. \gls*{aifsn} 7/3/2/2. \texttt{cwMin} 15/15/7/3. \texttt{cwMax} 1023/1023/15/7. \\
	\addlinespace
	\texttt{stable} &
	  Sustained VO &
	  Config \path{edca_v2x_vo_stable}. Inherits \texttt{edca\_only}; \path{V2xHcf}. Block~\SI{15}{\milli\second}; max cont.~\SI{80}{\milli\second}; guard~\SI{5}{\milli\second}; VO thr.~2. \\
	\addlinespace
	\texttt{guarded} &
	  Bounded VO &
	  Config \path{edca_v2x_vo_guarded}. Same inheritance. Block~\SI{4}{\milli\second}; max cont.~\SI{20}{\milli\second}; guard~\SI{4}{\milli\second}; VO thr.~3. \\
	\addlinespace
	\texttt{emergency} &
	  Emergency VO &
	  Config \path{edca_v2x_vo_emergency}; \texttt{emergencyPreemption=true}. Block~\SI{10}{\milli\second}; max cont.~\SI{60}{\milli\second}; guard~\SI{5}{\milli\second}; VO thr.~1; may drop BE. \\
\end{xltabular}
\fonte{Prepared by the author.}
}
...
\path{veins_qos/simulations/}~\cite{aguiar2026_veins_qos}:
...
\begin{itemize}
  ...
  \item obstacle loss typename \path{IdealObstacleLoss},\newline
        configured from \path{highway.obstacles.xml}~\cite{inet_ideal_obstacle_loss};
...
A sixth overlay family, \texttt{\_hotspot3}, concentrates application load spatially for the hotspot study of \autoref{sec:eval:hotspot}: the three vehicles that enter the corridor immediately after the crash source (nodes 1--3) bulk-upload at a \SI{4}{\milli\second} Poisson mean with 1200~B payloads (\(\approx\)\SI{2.4}{\mega\bit\per\second} each) for the whole run, independently of the crash window.
Composed as \texttt{<policy>\_hotspot\_<low|medium|high>}, with the overlay listed \emph{before} the \texttt{\_netload\_*} section because OMNeT++ keeps the first matching assignment, it adds fifteen more named configurations to the heavy package.
...
\begin{table}[htb]
	...
	\begin{center}
		\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{4.2cm} >{\raggedright\arraybackslash}X@{}}
			...
			\midrule
			\path{CritPacketSender} &
			  BE TX/RX counts\newline
			  VO RX counts\newline
			  BE and VO end-to-end delay vectors with count/mean/min/max scalars \\
			\addlinespace
			\path{CrashBurstApp} &
			  Physical VO TX count\newline
			  Logical VO sequence count \\
			\addlinespace
			\path{V2xIeee80211Mac} &
			  Per-\gls*{ac} drop totals\newline
			  Per-reason drop totals (overflow, retry limit, congestion, \ldots) \\
			\addlinespace
			\path{V2xHcf} &
			  Best Effort drops while blocked\newline
			  Best Effort grant suppressions\newline
			  Voice protection activations \\
			\addlinespace
			\path{V2xEdcaFsmController} &
			  \path{v2xState} vector (listening/blocking/sending) \\
			\bottomrule
		\end{tabularx}
	\end{center}
	\fonte{Prepared by the author.}
...
Crash Voice stamps creation at the logical burst start (\texttt{logicalCreationTime}), so the Voice delay sample is alert age---the time from the first attempt of that sequence until a receiver's first accepted copy---rather than the air time of a later physical repeat~\cite{aguiar2026_veins_qos}.
Alert age consequently includes the per-copy dispatch jitter described in \autoref{sec:impl:crash}, which is the intended semantics (the alert is only as timely as the application makes it) but must be kept in mind when absolute Voice delays are read.
Receivers also record a crash-node Best Effort slice (\texttt{beEndToEndDelayFromCrash}) for packets whose source is the designated crash vehicle, so Voice can be compared with Best Effort from the same sender.
...
Older packages under \path{simulations/veins_inet_square/},
\path{veins_inet_highway/}, and \path{veins_inet_light/}
