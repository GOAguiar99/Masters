...
Cooperative vehicular applications depend on the timely wireless dissemination of safety-relevant information among nearby vehicles. \gls{v2x} communication enables vehicles to exchange messages directly with other vehicles, infrastructure, and pedestrians, and \gls{ieee}~802.11 remains an important reference technology for this purpose because it supports direct, decentralized exchanges without continuous infrastructure support \cite{jiang2008ieee80211p,IEEE80211}. In particular, IEEE~802.11p adapts the standard for vehicular operation, typically at 5.9~GHz with 10~MHz channelization, and relies on fully distributed channel access: there is no central scheduler that can reserve transmission opportunities for urgent traffic.
...
\begin{enumerate}
	...
	\item To evaluate the five policies across a matrix of vehicle densities (10 and 100 vehicles), offered-load profiles, and spatial concentrations of that load---from a uniform fleet-wide offer to a localized hotspot near the crash source---using class-specific delay, jitter, multicast reach, and MAC-loss metrics.
...
\begin{enumerate}
	...
	\item Engineering guidance on \emph{when} adaptive suppression should be enabled, expressed as a regime-conditional protection-versus-cost frontier: under a uniform Poisson offer, Voice alert age does not separate the five \gls{mac} policies (about 2.2~ms P95), and blocking without dropping only inflates Best Effort delay (\texttt{stable}: 0.51~ms to 30~ms P95); but under concentrated near-crash contention with dense alert bursts, emergency preemption recovers Voice service (about $-39\%$ mean and $-21\%$ P95 alert age versus \gls{dcf}, with $30\%$ fewer Voice collision losses than plain \gls{edca})---supporting event-triggered escalation rather than always-on suppression.
\end{enumerate}

\section{Document Structure}
\label{sec:intro:structure}

The remainder of this dissertation is structured as follows. \autoref{chap:background} provides the necessary background on IEEE~802.11p, contention-based channel access, EDCA, and the DSCP-to-Voice mapping path. \autoref{sec:rel_work} surveys related work on vehicular QoS and stronger contention-control alternatives. \autoref{chap:sysmodel} formalizes the system model: the highway scenario, the two-class traffic abstraction, the crash event timeline, and the five MAC policies. \autoref{chap:implementation} details the realization in \gls{omnet}/Veins/\gls{inet}. \autoref{chap:evaluation} reports the evaluation results and their discussion. \autoref{chap:conclusion} concludes.
