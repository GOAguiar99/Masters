...
\begin{enumerate}
	...
	\item Engineering guidance on \emph{when} adaptive suppression should be enabled: at the Poisson loads exercised here, Voice alert age does not separate the five \gls{mac} policies (about 2.2~ms P95), while bounded blocking inflates Best Effort delay (\texttt{stable}: 0.51~ms to 30~ms P95) and emergency converts backlog into explicit Best Effort drops---supporting event-triggered escalation rather than always-on suppression.
\end{enumerate}
